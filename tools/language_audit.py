...
NOTE_OVERRIDES = {
    "ARRAY ADD": "Portable in principle, but requires a reviewed array helper subsystem covering numeric/string arrays, cardinality, and stride semantics.",
    "ARRAY INSERT": "Portable in principle, but requires a reviewed array helper subsystem covering multidimensional slice/index semantics.",
    "ARRAY SET": "Portable in principle, but requires a reviewed array helper subsystem covering numeric/string arrays and type coercion.",
    "ARRAY SLICE": "Portable in principle, but requires a reviewed array helper subsystem covering multidimensional slice/index semantics.",
    "ASTRO": "Portable in principle, but implemented in PicoMite's GPS/astronomy code and needs a dedicated source review plus domain tests.",
    ...
    "BEZIER": "Portable graphics algorithm in principle, but needs a dedicated graphics-backend port and visual/regression tests.",
    ...
    "END TYPE": "Part of PicoMite v6 structured-type support; requires a parser/runtime design rather than a standalone command token.",
    "FILL": "Portable graphics algorithm in principle, but needs a dedicated graphics-backend port and visual/regression tests.",
    "FLAGS": "`Flags = value` is implemented, but bare `Flags` function readback is blocked by the one-byte function token table being full.",
    "HELP": "PicoMite reads `A:/help.txt`; MMB4L should map this to local docs or command/function lists if implemented.",
    "LINPUT": "Handler is portable in principle, but adding `LInput(` as a function is blocked by the one-byte function token table being full.",
    ...
    "LOCATION": "Portable in principle, but implemented in PicoMite's GPS/astronomy code and needs a dedicated source review plus domain tests.",
    "MANDELBROT": "Portable graphics algorithm in principle, but needs a dedicated graphics-backend port and visual/regression tests.",
    "MSGBOX": "GUI popup helper; needs an SDL/Linux UI decision before implementation.",
    "PIXEL": "PicoMite `Pixel(x,y)` reads a pixel colour, but adding it as a function is blocked by the one-byte function token table being full.",
    "REDIM": "Portable in principle, but requires reviewed runtime support for resizing existing arrays without corrupting variable-table storage.",
    ...
    "STAR": "Portable in principle, but implemented in PicoMite's GPS/astronomy code and needs a dedicated source review plus domain tests.",
    "STRUCT": "PicoMite structured-type support needs parser, variable-table, and token-table architecture work before this surface can be implemented.",
    "TURTLE": "Portable graphics algorithm in principle, but needs a dedicated graphics-state/backend port and visual/regression tests.",
    "TYPE": "Part of PicoMite v6 structured-type support; requires a parser/runtime design rather than a standalone command token.",
    "VAR": "PicoMite `VAR` persists variables in flash; MMB4L needs explicit Linux filesystem persistence semantics before implementation.",
